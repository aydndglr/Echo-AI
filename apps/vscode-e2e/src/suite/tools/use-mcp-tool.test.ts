...
import { RooCodeEventName, type ClineMessage } from "@echo-ai/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("Echo AI use_mcp_tool Tool", function () {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		await sleep(2000) // Wait for Echo AI to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this Echo AI won't load the MCP server
