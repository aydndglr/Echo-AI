...
/* -----------------------------------------------------------------------------
   YENİ NO-OP IMPLEMENTATION
   - Hiçbir network isteği yapılmaz
   - Hiçbir veri gönderilmez
   - Tüm public API imzaları korunur (derleme ve kullanım bozulmasın diye)
   ----------------------------------------------------------------------------- */
...
abstract class BaseTelemetryClient implements TelemetryClient {
	...
	// Event filtresi (artık hiçbir şeyi umursamıyoruz, ama imza dursun)
	protected isEventCapturable(_eventName: TelemetryEventName): boolean {
		return false
	}

	// Özellik filtresi (kullanılmıyor, sadece tip uyumu)
	protected isPropertyCapturable(_propertyName: string): boolean {
		return false
	}

	protected async getEventProperties(_event: TelemetryEvent): Promise<TelemetryEvent["properties"]> {
		// Artık sağlayıcıdan property çekmiyoruz, tamamen boş
		return {}
	}
	...
	public setProvider(provider: TelemetryPropertiesProvider): void {
		// Provider referansını tutabiliriz ama kullanmayacağız
...
export class CloudTelemetryClient extends BaseTelemetryClient {
	// Orijinal constructor imzasını koruyoruz (AuthService, SettingsService, RetryQueue)
	constructor(
		_authService: AuthService,
		_settingsService: SettingsService,
		_retryQueue?: unknown,
	) {
		// Artık subscription veya debug kullanmıyoruz
		super(undefined, false)
		this.telemetryEnabled = false
	}

	public override async capture(_event: TelemetryEvent): Promise<void> {
		// NO-OP: hiçbir event gönderilmiyor
		if (this.debug) {
			// İstersen debug log bırakabilirsin ama tavsiye etmiyorum
			// console.info(`[CloudTelemetryClient#capture] Telemetry disabled, event skipped: ${_event.event}`)
		}
		return
	}

	public async backfillMessages(_messages: ClineMessage[], _taskId: string): Promise<void> {
		// Orijinal API imzasını koruyoruz ama hiçbir şey yapmıyoruz
		if (this.debug) {
			// console.info(`[CloudTelemetryClient#backfillMessages] Telemetry disabled, skipping backfill`)
		}
		return
	}

	public override updateTelemetryState(_didUserOptIn: boolean): void {
		// Kullanıcı izin verse bile telemetry devre dışı
		this.telemetryEnabled = false
	}
...
/* -----------------TÜM İZLEMELERİ KAPATIYORUZ----------------------------------

/* -----------------------------------------------------------------------------
   AŞAĞIDAKİ BLOK: ORİJİNAL KODUN TAMAMI (DEVRE DIŞI)
   - Referans istersen diye bırakıldı
   - Derleme sırasında tamamen yorum satırı olarak yok sayılır
   ----------------------------------------------------------------------------- */
...
// 		if (!result.success) {
// 			console.error(
// 				`[TelemetryClient#capture] Invalid telemetry event: ${result.error.message} - ${JSON.stringify(
// 					payload,
// 				)}`,
// 			)
//
