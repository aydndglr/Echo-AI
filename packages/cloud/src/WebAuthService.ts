...
export class WebAuthService extends EventEmitter<AuthServiceEvents> implements AuthService {
	...
	public async login(landingPageSlug?: string, useProviderSignup: boolean = false): Promise<void> {
		try {
			...
			this.log(`[auth] Error initiating Echo AI Cloud auth${context}: ${error}`)
			throw new Error(`Failed to initiate Echo AI Cloud authentication${context}: ${error}`)
		}
	}

	/**
	 * Handle the callback from Echo AI Cloud
	 *
	 * This method is called when the user is redirected back to the extension
	 * after authenticating with Echo AI Cloud.
	 *
	...
	public async handleCallback(
		...
		if (!code || !state) {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Invalid Echo AI Cloud sign in url")
			}
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Successfully authenticated with Echo AI Cloud")
			}

			this.log("[auth] Successfully authenticated with Echo AI Cloud")
		} catch (error) {
			this.log(`[auth] Error handling Echo AI Cloud callback: ${error}`)
			this.changeState("logged-out")
			throw new Error(`Failed to handle Echo AI Cloud callback: ${error}`)
		}
	}
	...
	public async logout(): Promise<void> {
		...
		try {
			...
			if (vscode) {
				vscode.window.showInformationMessage("Logged out from Echo AI Cloud")
			}

			this.log("[auth] Logged out from Echo AI Cloud")
		} catch (error) {
			this.log(`[auth] Error logging out from Echo AI Cloud: ${error}`)
			throw new Error(`Failed to log out from Echo AI Cloud: ${error}`)
		}
	}
