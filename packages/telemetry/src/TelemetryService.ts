...
// -----------------------------------------------------------------------------
// ORİJİNAL TelemetryService KODU (TAMAMI DEVRE DIŞI)
// -----------------------------------------------------------------------------

// /**
//  * TelemetryService
//  * (Orijinal, artık kullanılmıyor. Referans için tutuluyor.)
//  */
...
// -----------------------------------------------------------------------------
// YENİ NO-OP TelemetryService
// Tüm capture çağrıları burada sessizce yutuluyor, HİÇBİR veri gönderilmiyor.
// -----------------------------------------------------------------------------

/**
 * NO-OP TelemetryService
 * Tüm telemetry çağrıları sessizce yutulur, hiçbir yere gönderilmez.
 */
export class TelemetryService {
	private static _instance: TelemetryService | null = null

	private constructor(private clients: TelemetryClient[] = []) {}

	// Singleton oluşturma (orijinal imzayla uyumlu)
	static createInstance(clients: TelemetryClient[] = []) {
		if (this._instance) {
			throw new Error("TelemetryService instance already created")
		}
	...
	// Singleton instance getter (orijinal imzayla uyumlu)
	static get instance() {
		if (!this._instance) {
			throw new Error("TelemetryService not initialized")
		}
	...
	public register(_client: TelemetryClient): void {
		// İstersen burada this.clients.push(_client) diyebilirsin ama telemetry kapalı olduğu için gerek yok.
	}
	...
	private get isReady(): boolean {
		// Artık hiçbir client aktif değil, ama imza açısından bu fonksiyon kalsın.
		return this.clients.length > 0
	}

	public updateTelemetryState(_isOptedIn: boolean): void {
		// Kullanıcı telemetry açsa bile, biz hiçbir şey yapmıyoruz.
	}
	...
	public captureToolUsage(_taskId: string, _tool: string, _toolProtocol: string): void {}
	...
	public captureContextCondensed(
		_taskId: string,
		_isAutomaticTrigger: boolean,
		_usedCustomPrompt?: boolean,
		_usedCustomApiHandler?: boolean,
	): void {}
	...
	public captureSchemaValidationError(_args: { schemaName: string; error: ZodError }): void {}
	...
	public isTelemetryEnabled(): boolean {
		// Her zaman false, çünkü artık hiçbir telemetry gönderilmiyor.
		return false
	}
