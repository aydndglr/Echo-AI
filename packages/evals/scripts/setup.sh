...
build_extension() {
  echo "🔨 Building the Echo AI extension..."
...
if [[ "$(uname -s)" != "Darwin" ]]; then
  ...
  echo "The Echo AI evals system can also be run with Docker on any platform."
...
echo -n "🗄️ Syncing Echo AI evals database... "
pnpm --filter @echo-ai/evals db:push --force &>/dev/null || exit 1
echo "✅ Done"
...
read -p "💻 Do you want to build a new version of the Echo AI extension? [currently $current_version] (y/N): " build_extension
...
if ! nc -z localhost 3446; then
  ...
  if [[ "$start_evals" =~ ^[Yy]|^$ ]]; then
    pnpm --filter @echo-ai/web-evals dev
  else
    echo "💡 You can start it anytime with 'pnpm --filter @echo-ai/web-evals dev'."
  fi
else
