...
import { CloudService } from "@echo-ai/cloud"
...
export const webviewMessageHandler = async (
	...
	switch (message.type) {
		...
		case "updateSettings":
			if (message.updatedSettings) {
				for (const [key, value] of Object.entries(message.updatedSettings)) {
					...
					await provider.contextProxy.setValue(key as keyof EchoAiSettings, newValue)
				}
		...
		case "requestRouterModels":
			...
			const candidates: { key: RouterName; options: GetModelsOptions }[] = [
				...
				},
				{
					...
					options: {
						provider: "echo",
						baseUrl: process.env.ECHO_AI_PROVIDER_URL ?? "https://api.devlog.tr/proxy",
		...
		case "requestDevlogModels": {
			// Specific handler for Echo models only - flushes cache to ensure fresh auth token is used
			try {
				// Flush cache and refresh to ensure fresh models with current auth state
				await flushModels("echo", true)

				const echoModels = await getModels({
					provider: "echo",
					baseUrl: process.env.ECHO_AI_PROVIDER_URL ?? "https://api.devlog.tr/proxy",
					apiKey: CloudService.hasInstance()
						? CloudService.instance.authService?.getSessionToken()
				...
				provider.postMessageToWebview({
					type: "singleRouterModelFetchResponse",
					success: true,
					values: { provider: "echo", models: echoModels },
				})
			} catch (error) {
				// Send error response
				...
				provider.postMessageToWebview({
					...
					success: false,
					error: errorMessage,
					values: { provider: "echo" },
				})
			}
			break
		}
		case "requestDevlogCreditBalance": {
			// Fetch Echo credit balance using CloudAPI
		...
		case "openKeyboardShortcuts": {
			// Open VSCode keyboard shortcuts settings and optionally filter to show the Echo AI commands
		...
		case "openProjectMcpSettings": {
			...
			const echoDir = path.join(workspaceFolder, ".echo")
			const mcpPath = path.join(echoDir, "mcp.json")
		...
		case "deleteCustomMode":
			if (message.slug) {
				...
				if (scope === "project") {
					...
					if (workspacePath) {
						rulesFolderPath = path.join(workspacePath, ".echo", `rules-${message.slug}`)
					} else {
						rulesFolderPath = path.join(".echo", `rules-${message.slug}`)
					}
				} else {
					// Global scope - use OS home directory
					const homeDir = os.homedir()
					rulesFolderPath = path.join(homeDir, ".echo", `rules-${message.slug}`)
				}
		...
		case "createCommand": {
			try {
				...
				if (source === "global") {
					const globalConfigDir = path.join(os.homedir(), ".echo")
					...
					commandsDir = path.join(workspaceRoot, ".echo", "commands")
				}
