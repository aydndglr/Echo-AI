...
import { CloudService, BridgeOrchestrator, getRooCodeApiUrl } from "@echo-ai/cloud"
...
import type { ClineMessage } from "@echo-ai/types"
	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider, TaskProviderLike
{
	...
	constructor(
		...
		this.taskCreationCallback = (instance: Task) => {
			this.emit(EchoCodeEventName.TaskCreated, instance)

			// Create named listener functions so we can remove them later.
			const onTaskStarted = () => this.emit(EchoCodeEventName.TaskStarted, instance.taskId)
			const onTaskCompleted = (taskId: string, tokenUsage: any, toolUsage: any) =>
				this.emit(EchoCodeEventName.TaskCompleted, taskId, tokenUsage, toolUsage)
			const onTaskAborted = async () => {
				this.emit(EchoCodeEventName.TaskAborted, instance.taskId)
			...
			const onTaskFocused = () => this.emit(EchoCodeEventName.TaskFocused, instance.taskId)
			const onTaskUnfocused = () => this.emit(EchoCodeEventName.TaskUnfocused, instance.taskId)
			const onTaskActive = (taskId: string) => this.emit(EchoCodeEventName.TaskActive, taskId)
			const onTaskInteractive = (taskId: string) => this.emit(EchoCodeEventName.TaskInteractive, taskId)
			const onTaskResumable = (taskId: string) => this.emit(EchoCodeEventName.TaskResumable, taskId)
			const onTaskIdle = (taskId: string) => this.emit(EchoCodeEventName.TaskIdle, taskId)
			const onTaskPaused = (taskId: string) => this.emit(EchoCodeEventName.TaskPaused, taskId)
			const onTaskUnpaused = (taskId: string) => this.emit(EchoCodeEventName.TaskUnpaused, taskId)
			const onTaskSpawned = (taskId: string) => this.emit(EchoCodeEventName.TaskSpawned, taskId)
			const onTaskUserMessage = (taskId: string) => this.emit(EchoCodeEventName.TaskUserMessage, taskId)
			const onTaskTokenUsageUpdated = (taskId: string, tokenUsage: TokenUsage) =>
				this.emit(EchoCodeEventName.TaskTokenUsageUpdated, taskId, tokenUsage)

			// Attach the listeners.
			instance.on(EchoCodeEventName.TaskStarted, onTaskStarted)
			instance.on(EchoCodeEventName.TaskCompleted, onTaskCompleted)
			instance.on(EchoCodeEventName.TaskAborted, onTaskAborted)
			instance.on(EchoCodeEventName.TaskFocused, onTaskFocused)
			instance.on(EchoCodeEventName.TaskUnfocused, onTaskUnfocused)
			instance.on(EchoCodeEventName.TaskActive, onTaskActive)
			instance.on(EchoCodeEventName.TaskInteractive, onTaskInteractive)
			instance.on(EchoCodeEventName.TaskResumable, onTaskResumable)
			instance.on(EchoCodeEventName.TaskIdle, onTaskIdle)
			instance.on(EchoCodeEventName.TaskPaused, onTaskPaused)
			instance.on(EchoCodeEventName.TaskUnpaused, onTaskUnpaused)
			instance.on(EchoCodeEventName.TaskSpawned, onTaskSpawned)
			instance.on(EchoCodeEventName.TaskUserMessage, onTaskUserMessage)
			instance.on(EchoCodeEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated)

			// Store the cleanup functions for later removal.
			this.taskEventListeners.set(instance, [
				() => instance.off(EchoCodeEventName.TaskStarted, onTaskStarted),
				() => instance.off(EchoCodeEventName.TaskCompleted, onTaskCompleted),
				() => instance.off(EchoCodeEventName.TaskAborted, onTaskAborted),
				() => instance.off(EchoCodeEventName.TaskFocused, onTaskFocused),
				() => instance.off(EchoCodeEventName.TaskUnfocused, onTaskUnfocused),
				() => instance.off(EchoCodeEventName.TaskActive, onTaskActive),
				() => instance.off(EchoCodeEventName.TaskInteractive, onTaskInteractive),
				() => instance.off(EchoCodeEventName.TaskResumable, onTaskResumable),
				() => instance.off(EchoCodeEventName.TaskIdle, onTaskIdle),
				() => instance.off(EchoCodeEventName.TaskUserMessage, onTaskUserMessage),
				() => instance.off(EchoCodeEventName.TaskPaused, onTaskPaused),
				() => instance.off(EchoCodeEventName.TaskUnpaused, onTaskUnpaused),
				() => instance.off(EchoCodeEventName.TaskSpawned, onTaskSpawned),
				() => instance.off(EchoCodeEventName.TaskTokenUsageUpdated, onTaskTokenUsageUpdated),
			])
		}

		// Initialize Echo AI Cloud profile sync.
	...
	async addClineToStack(task: Task) {
		...
		task.emit(EchoCodeEventName.TaskFocused)
	...
	async removeClineFromStack() {
		...
		if (task) {
			task.emit(EchoCodeEventName.TaskUnfocused)
	...
	*/
	...
	public async handleModeSwitch(newMode: Mode) {
		...
		if (task) {
			...
			task.emit(EchoCodeEventName.TaskModeSwitched, task.taskId, newMode)
		...
		this.emit(EchoCodeEventName.ModeChanged, newMode)
	...
	async activateProviderProfile(args: { name: string } | { id: string }) {
		...
		if (providerSettings.apiProvider) {
			this.emit(EchoCodeEventName.ProviderProfileChanged, { name, provider: providerSettings.apiProvider })
		}
	}
	...
	public async setValue<K extends keyof EchoAiSettings>(key: K, value: EchoAiSettings[K]) {
		await this.contextProxy.setValue(key, value)
	}

	public getValue<K extends keyof EchoAiSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: EchoAiSettings) {
