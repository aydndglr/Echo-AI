...
import type { ModeConfig } from "@echo-ai/types"
...
describe("CustomModesManager", () => {
	...
	const mockRoomodes = path.join(mockWorkspacePath, ".echomodes")
	...
	describe("getCustomModes", () => {
		it("should handle valid YAML in .echomodes file and JSON for global customModes", async () => {
		...
		it("should merge modes with .echomodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .echomodes since it takes precedence
		...
		it("should handle missing .echomodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
		...
		it("should handle invalid YAML in .echomodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			...
			// Should fall back to settings modes when .echomodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .echomodes precedence", async () => {
			...
			// Should update global state with merged modes where .echomodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .echomodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .echomodes file when adding project-specific mode", async () => {
			...
			// Mock .echomodes to not exist initially
			...
			// Verify .echomodes was created with the project mode
			...
			// Verify the content written to .echomodes
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".echomodes"),
			...
			it("should handle import errors gracefully", async () => {
				...
				// Mock fs.writeFile to work normally for .echomodes but we won't get there
				;(fs.writeFile as Mock).mockResolvedValue(undefined)
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".echomodes"))
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .echomodes file", async () => {
		...
		it("should return false when .echomodes doesn't exist and mode is not a custom mode", async () => {
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
		...
		it("should work with global custom modes when .echomodes doesn't exist", async () => {
			...
			;(fileExistsAtPath as Mock).mockImplementation(async (path: string) => {
				return path === mockSettingsPath // .echomodes doesn't exist
			})
			;(fs.readFile as Mock).mockImplementation(async (path: string) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a custom mode in .echomodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .echomodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "code",
