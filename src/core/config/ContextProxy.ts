...
type EchoSettingsKey = keyof EchoAiSettings
...
export class ContextProxy {
	...
	public async setValue<K extends EchoSettingsKey>(key: K, value: EchoAiSettings[K]) {
	...
	public getValue<K extends EchoSettingsKey>(key: K): EchoAiSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key as SecretStateKey) as EchoAiSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as EchoAiSettings[K])
	}

	public getValues(): EchoAiSettings {
	...
	public async setValues(values: EchoAiSettings) {
		const entries = Object.entries(values) as [EchoSettingsKey, unknown][]
