...
import { CloudService, BridgeOrchestrator } from "@echo-ai/cloud"
...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	constructor({
		...
		this.rooProtectedController = new EchoProtectedController(this.cwd)
		...
		this.messageQueueStateChangedHandler = () => {
			this.emit(EchoCodeEventName.TaskUserMessage, this.taskId)
	...
	private setupProviderProfileChangeListener(provider: ClineProvider): void {
		...
		provider.on(EchoCodeEventName.ProviderProfileChanged, this.providerProfileChangeListener)
	}
	...
	private async addToClineMessages(message: ClineMessage) {
		...
		this.emit(EchoCodeEventName.Message, { action: "created", message })
	...
	private async updateClineMessage(message: ClineMessage) {
		...
		this.emit(EchoCodeEventName.Message, { action: "updated", message })
	...
	private async saveClineMessages() {
		try {
			...
			if (hasTokenUsageChanged(tokenUsage, this.tokenUsageSnapshot)) {
				this.emit(EchoCodeEventName.TaskTokenUsageUpdated, this.taskId, tokenUsage)
	...
	async ask(
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(EchoCodeEventName.TaskInteractive, this.taskId)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(EchoCodeEventName.TaskResumable, this.taskId)
						}
					}, statusMutationTimeout),
				)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(EchoCodeEventName.TaskIdle, this.taskId)
						}
					}, statusMutationTimeout),
				)
			}
		...
		if (this.idleAsk || this.resumableAsk || this.interactiveAsk) {
			...
			this.emit(EchoCodeEventName.TaskActive, this.taskId)
		}

		this.emit(EchoCodeEventName.TaskAskResponded)
		return result
	}
	...
	public async abortTask(isAbandoned = false) {
		...
		this.abort = true
		this.emit(EchoCodeEventName.TaskAborted)
	...
	public dispose(): void {
		...
		try {
			if (this.providerProfileChangeListener) {
				const provider = this.providerRef.deref()
				if (provider) {
					provider.off(EchoCodeEventName.ProviderProfileChanged, this.providerProfileChangeListener)
				}
	...
	public async startSubtask(message: string, initialTodos: TodoItem[], mode: string) {
		...
		if (newTask) {
			...
			this.emit(EchoCodeEventName.TaskPaused, this.taskId)
			this.emit(EchoCodeEventName.TaskSpawned, newTask.taskId)
		}
	...
	public async completeSubtask(lastMessage: string) {
		...
		this.emit(EchoCodeEventName.TaskUnpaused, this.taskId)
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		this.emit(EchoCodeEventName.TaskStarted)
	...
	public recordToolError(toolName: ToolName, error?: string) {
		...
		if (error) {
			this.emit(EchoCodeEventName.TaskToolFailed, this.taskId, toolName, error)
		}
	}
