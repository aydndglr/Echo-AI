...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .echoignore exists
		 */
		it("should load .echoignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .echoignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".echoignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".echoignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .echoignore doesn't exist
		 */
		it("should allow all access when .echoignore doesn't exist", async () => {
			// Setup mocks to simulate missing .echoignore file
		 ...
		 */
		it("should set up file watcher for .echoignore changes", async () => {
		 ...
		 */
		it("should handle errors when loading .echoignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .echoignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .echoignore exists
		 */
		it("should allow all access when no .echoignore content", async () => {
			// Create a new controller with no .echoignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .echoignore exists
		 */
		it("should allow all commands when no .echoignore exists", async () => {
			// Create a new controller with no .echoignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .echoignore
		 */
		it("should generate formatted instructions when .echoignore exists", async () => {
			// Setup .echoignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .echoignore exists
		 */
		it("should return undefined when no .echoignore exists", async () => {
			// Setup no .echoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .echoignore is created
		 */
		it("should reload .echoignore when file is created", async () => {
			// Setup initial state without .echoignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .echoignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .echoignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .echoignore is changed
		 */
		it("should reload .echoignore when file is changed", async () => {
			// Setup initial state with .echoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .echoignore is deleted
		 */
		it("should reset when .echoignore is deleted", async () => {
			// Setup initial state with .echoignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
