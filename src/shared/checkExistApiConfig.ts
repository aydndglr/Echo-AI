import { SECRET_STATE_KEYS, GLOBAL_SECRET_KEYS, ProviderSettings } from "@echo-ai/types"
...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	if (
		config.apiProvider &&
		["human-relay", "fake-ai", "claude-code", "qwen-code", "echo"].includes(config.apiProvider)
	) {
		return true
	}
