...
import { rooDefaultModelId } from "@echo-ai/types"
...
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key: string) => {
		if (key === "common:errors.roo.authenticationRequired") {
			return "Authentication required for Echo AI Cloud"
		}
		return key
	}),
}))
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"Echo AI Cloud completion error: API Error",
			)
		})
