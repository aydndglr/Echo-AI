import { EchoModelsResponseSchema } from "@echo-ai/types"
...
/**
 * Fetches available models from the Echo AI Cloud provider
 *
 * @param baseUrl The base URL of the Echo AI Cloud provider
 * @param apiKey The API key (session token) for the Echo AI Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			const parsed = EchoModelsResponseSchema.safeParse(data)

			if (!parsed.success) {
				console.error("Error fetching Echo AI Cloud models: Unexpected response format", data)
				console.error("Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch Echo AI Cloud models: Unexpected response format.")
			}
		...
		console.error("[getRooModels] Error fetching Echo AI Cloud models:", {
		...
		if (error.name === "AbortError") {
			throw new Error("Failed to fetch Echo AI Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (error.message?.includes("HTTP")) {
			throw new Error(`Failed to fetch Echo AI Cloud models: ${error.message}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch Echo AI Cloud models: No response from server. Check Echo AI Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch Echo AI Cloud models: ${error.message || "An unknown error occurred."}`)
	}
}
