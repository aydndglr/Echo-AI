...
import { echoDefaultModelId, getApiProtocol, type ImageGenerationApiMethod } from "@echo-ai/types"
...
import type { RooReasoningParams as EchoReasoningParams } from "../transform/reasoning"
...
import type { ModelInfo } from "@echo-ai/types"
...
interface EchoUsage extends OpenAI.CompletionUsage {
	cache_creation_input_tokens?: number
	cost?: number
}

// Add custom interface for Roo params to support reasoning
type EchoChatCompletionParams = OpenAI.Chat.ChatCompletionCreateParamsStreaming & {
	reasoning?: EchoReasoningParams
}
...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	constructor(options: ApiHandlerOptions) {
		...
		let baseURL = process.env.ECHO_AI_PROVIDER_URL ?? "https://api.devlog.tr/proxy"
		...
		super({
			...options,
			providerName: "Echo AI Cloud",
			baseURL, // Already has /v1 suffix
			apiKey: sessionToken,
			defaultProviderModelId: echoDefaultModelId,
			providerModels: {},
	...
	protected override createStream(
		...
		const echoParams: EchoChatCompletionParams = {
			model,
			max_tokens,
			temperature,
		...
		try {
			...
			return this.client.chat.completions.create(echoParams, requestOptions)
		} catch (error) {
	...
	override async *createMessage(
		...
		try {
			...
			let lastUsage: EchoUsage | undefined = undefined
			...
			for await (const chunk of stream) {
				...
				if (chunk.usage) {
					lastUsage = chunk.usage as EchoUsage
				}
			}
			...
			if (lastUsage) {
				...
				const apiProtocol = getApiProtocol("echo", modelId)
	...
	override getModel() {
		const modelId = this.options.apiModelId || echoDefaultModelId

		// Get models from shared cache
		const models = getModelsFromCache("echo") || {}
	...
	/**
	 * Generate an image using Echo AI Cloud's image generation API
	...
	async generateImage(
		...
		if (!sessionToken || sessionToken === "unauthenticated") {
			return {
				success: false,
				error: t("tools:generateImage.echo.authRequired"),
			}
		}
