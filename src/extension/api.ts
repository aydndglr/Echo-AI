...
import { IpcServer } from "@echo-ai/ipc"
...
export class API extends EventEmitter<RooCodeEvents> implements RooCodeAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(os.tmpdir(), "echo-ai-messages.log")
		} else {
			this.log = () => {}
		}
	...
	public override emit<K extends keyof RooCodeEvents>(
		...
		const data = { eventName: eventName as EchoCodeEventName, payload: args } as TaskEvent
	...
	private registerListeners(provider: ClineProvider) {
		provider.on(EchoCodeEventName.TaskCreated, (task) => {
			// Task Lifecycle

			task.on(EchoCodeEventName.TaskStarted, async () => {
				this.emit(EchoCodeEventName.TaskStarted, task.taskId)
				this.taskMap.set(task.taskId, provider)
				await this.fileLog(`[${new Date().toISOString()}] taskStarted -> ${task.taskId}\n`)
			})

			task.on(EchoCodeEventName.TaskCompleted, async (_, tokenUsage, toolUsage) => {
				this.emit(EchoCodeEventName.TaskCompleted, task.taskId, tokenUsage, toolUsage, {
			...
			task.on(EchoCodeEventName.TaskAborted, () => {
				this.emit(EchoCodeEventName.TaskAborted, task.taskId)
				this.taskMap.delete(task.taskId)
			})

			task.on(EchoCodeEventName.TaskFocused, () => {
				this.emit(EchoCodeEventName.TaskFocused, task.taskId)
			})

			task.on(EchoCodeEventName.TaskUnfocused, () => {
				this.emit(EchoCodeEventName.TaskUnfocused, task.taskId)
			})

			task.on(EchoCodeEventName.TaskActive, () => {
				this.emit(EchoCodeEventName.TaskActive, task.taskId)
			})

			task.on(EchoCodeEventName.TaskInteractive, () => {
				this.emit(EchoCodeEventName.TaskInteractive, task.taskId)
			})

			task.on(EchoCodeEventName.TaskResumable, () => {
				this.emit(EchoCodeEventName.TaskResumable, task.taskId)
			})

			task.on(EchoCodeEventName.TaskIdle, () => {
				this.emit(EchoCodeEventName.TaskIdle, task.taskId)
			})

			// Subtask Lifecycle

			task.on(EchoCodeEventName.TaskPaused, () => {
				this.emit(EchoCodeEventName.TaskPaused, task.taskId)
			})

			task.on(EchoCodeEventName.TaskUnpaused, () => {
				this.emit(EchoCodeEventName.TaskUnpaused, task.taskId)
			})

			task.on(EchoCodeEventName.TaskSpawned, (childTaskId) => {
				this.emit(EchoCodeEventName.TaskSpawned, task.taskId, childTaskId)
			})

			// Task Execution

			task.on(EchoCodeEventName.Message, async (message) => {
				this.emit(EchoCodeEventName.Message, { taskId: task.taskId, ...message })
			...
			task.on(EchoCodeEventName.TaskModeSwitched, (taskId, mode) => {
				this.emit(EchoCodeEventName.TaskModeSwitched, taskId, mode)
			})

			task.on(EchoCodeEventName.TaskAskResponded, () => {
				this.emit(EchoCodeEventName.TaskAskResponded, task.taskId)
			})

			// Task Analytics

			task.on(EchoCodeEventName.TaskToolFailed, (taskId, tool, error) => {
				this.emit(EchoCodeEventName.TaskToolFailed, taskId, tool, error)
			})

			task.on(EchoCodeEventName.TaskTokenUsageUpdated, (_, usage) => {
				this.emit(EchoCodeEventName.TaskTokenUsageUpdated, task.taskId, usage)
			})

			// Let's go!

			this.emit(EchoCodeEventName.TaskCreated, task.taskId)
		})
	}
	...
	public getConfiguration(): EchoAiSettings {
		return Object.fromEntries(
			Object.entries(this.sidebarProvider.getValues()).filter(([key]) => !isSecretStateKey(key)),
		)
	}

	public async setConfiguration(values: EchoAiSettings) {
