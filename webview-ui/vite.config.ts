...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("Echo-AI"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use `@echo-ai/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("Echo-AI-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@echo": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
