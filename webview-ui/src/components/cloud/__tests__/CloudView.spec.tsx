	...
	useAppTranslation: () => ({
		t: (key: string) => {
			const translations: Record<string, string> = {
				...
				"cloud:signIn": "Connect to Echo AI Cloud",
				"cloud:cloudBenefitsTitle": "Connect to Echo AI Cloud",
				...
				"cloud:visitCloudWebsite": "Visit Echo AI Cloud",
				"cloud:taskSync": "Task sync",
				"cloud:taskSyncDescription": "Sync your tasks for viewing and sharing on Echo AI Cloud",
				...
				"cloud:remoteControlDescription":
					"Enable following and interacting with tasks in this workspace with Echo AI Cloud",
				...
				"cloud:cloudUrlPillLabel": "Echo AI Cloud URL: ",
			}
...
describe("CloudView", () => {
	it("should display benefits when user is not authenticated", () => {
		...
		expect(screen.getByRole("heading", { name: "Connect to Echo AI Cloud" })).toBeInTheDocument()
	...
	it("should display remote control toggle when user has extension bridge enabled and roomote control enabled", () => {
		...
		expect(
			screen.getByText("Enable following and interacting with tasks in this workspace with Echo AI Cloud"),
		).toBeInTheDocument()
	})
	...
	it("should not display cloud URL pill when pointing to production", () => {
		...
		expect(screen.queryByText(/Echo AI Cloud URL:/)).not.toBeInTheDocument()
	})
	...
	it("should display cloud URL pill when pointing to non-production environment", () => {
		...
		expect(screen.getByText(/Echo AI Cloud URL:/)).toBeInTheDocument()
