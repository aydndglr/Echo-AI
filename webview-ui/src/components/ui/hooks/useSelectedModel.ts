...
import type { ModelRecord, RouterModels } from "@echo/api"
...
function getSelectedModel({
	...
	switch (provider) {
		...
		case "echo": {
			const id = getValidatedModelId(apiConfiguration.apiModelId, routerModels.echo, defaultModelId)
			const info = routerModels.echo?.[id]
			return { id, info }
		}
		case "qwen-code": {
			const id = apiConfiguration.apiModelId ?? defaultModelId
		...
		// case "fake-ai":
		default: {
			provider satisfies "anthropic" | "gemini-cli" | "qwen-code" | "human-relay" | "fake-ai" | "devlog"
			const id = apiConfiguration.apiModelId ?? defaultModelId
