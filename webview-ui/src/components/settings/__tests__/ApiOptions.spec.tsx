...
import { type ModelInfo, type ProviderSettings, openAiModelInfoSaneDefaults } from "@echo-ai/types"
...
describe("ApiOptions", () => {
	...
	describe("Roo provider tests", () => {
		...
		it("pins roo provider to the top when not on welcome screen", () => {
			...
			// Find the roo option
			const rooOption = providerOptions.find((opt) => opt.value === "echo")

			// If roo is available, verify it's pinned to the top
			if (rooOption) {
				expect(providerOptions[0].value).toBe("echo")
			}
		...
		it("does not pin roo provider to the top on welcome screen", () => {
			...
			if (rooOption) {
				...
				expect(providerOptions[0].value).not.toBe("echo")
			}
